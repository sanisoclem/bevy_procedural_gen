...
pub fn generate_mesh(thread_pool: &Res<AsyncComputeTaskPool>, _voxels: &HashMap<VoxelId, VoxelType>, _lod: u8) -> Task<Mesh> {
  ...
  // and if we can edit, we need to make sure that we don't edit while we are using it to generate the mesh
  // hmmm... maybe we need some sort of double buffer?
  ...
  thread_pool.spawn(async move {
    Mesh::from(shape::Plane { size: 1.0 * 23. })
  })
}
